...
void BTree<K,V>::BTreeNode::insert_non_full(BTree* bTree, const EntryT& entry) {
    ...
    if (is_leaf()) {
        ...
        int idx = find_key_bin_search(bTree, entry.key);

        Node node = get_node(bTree, idx);

        if (node.is_full()) {
            split_child(bTree, idx, node);

            Entry entryTmp = read_entry(bTree, idx);
...
Entry<K, V> BTree<K,V>::BTreeNode::find(BTree* bTree, const K& key) {
    ...
    if (idx < used_keys) {
        auto entry = read_entry(bTree, idx);
        if (entry.key == key) {
            return entry;
        }
    }
...
bool BTree<K,V>::BTreeNode::remove(BTree* bTree, const K& key) {
    ...
    if (idx < used_keys && entry.key == key) {
        success = is_leaf() ? remove_from_leaf(bTree, idx) : remove_from_non_leaf(bTree, idx);
        bTree->write_node(*this, m_pos);
        return success;
    } else if (!is_leaf()) {
        Node node = get_node(bTree, idx);
