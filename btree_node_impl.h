...
void BTree<K,V>::BTreeNode::insert_non_full(IOManagerT& io_manager, const EntryT& entry) {
    int i = used_keys - 1;
    Entry<K, V> entryTmp = read_entry(io_manager, i);

    if (is_leaf()) {
        while (i >= 0 && entryTmp.key > entry.key) {
            arrayPosKey[i + 1] = arrayPosKey[i];
            i--;
            entryTmp = read_entry(io_manager, i);
        }
        ...
        i = find_key_bin_search(io_manager, entry.key);

        Node node = get_node(io_manager, i);

        if (node.is_full()) {
            split_child(io_manager, i, node);
...
Entry<K, V> BTree<K,V>::BTreeNode::find(IOManagerT& io_manager, const K& key) {
    int i = 0;
    EntryT entry = read_entry(io_manager, i);
    ...
    return get_node(io_manager, i).find(io_manager, key);
}
...
bool BTree<K,V>::BTreeNode::remove(IOManagerT& io_manager, const K& key) {
    ...
    if (idx < used_keys && entry.key == key) {
        res = is_leaf() ? remove_from_leaf(io_manager, idx) : remove_from_non_leaf(io_manager, idx);
        io_manager.write_node(*this, m_pos);
    } else {
        if (is_leaf()) {
            return false;
        ...
        auto child = get_node(io_manager, idx);
        ...
        auto m_child  = get_node(io_manager, child_idx);

        if (m_child.t != 1) {
            res =  m_child.remove(io_manager, key);
            io_manager.write_node(m_child, m_child.m_pos);
        }
    }
